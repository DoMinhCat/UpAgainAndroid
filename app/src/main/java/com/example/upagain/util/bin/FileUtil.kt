...
fun saveBarcodeImage(context: Context, base64Str: String, transactionId: String, main: View) {
    try {
        ...
        if (imageUri != null) {
            ...
            main.showTopSnackbar(
                context.getString(R.string.barcode_saved_success),
                SnackbarLevel.SUCCESS
            )
        } else {
            main.showTopSnackbar(
                context.getString(R.string.barcode_saved_fail, ""),
                SnackbarLevel.ERROR
            )
        }
    } catch (e: Exception) {
        ...
        main.showTopSnackbar(
            context.getString(R.string.barcode_saved_fail, e.message ?: ""),
            SnackbarLevel.ERROR
        )
    }
}